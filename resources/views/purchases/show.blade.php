<x-shop-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Goods Receipt Details #{{ $purchase->id }}
            </h2>
    ...
    <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <dl class="space-y-3">
                            ...
                            <div>
                                ...
                                <dd class="text-sm text-gray-900">{{ $purchase->product->category?->name ?: 'Uncategorized' }}</dd>
                            </div>
                            <div>
                                <dt class="text-sm font-medium text-gray-500">SKU Code</dt>
                                <dd class="text-sm text-gray-900">{{ $purchase->product->sku ?: 'Not assigned' }}</dd>
                            </div>
                            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-600">Storage Location:</span>
                                <span class="text-sm font-medium">{{ $purchase->product->storage_location ?: 'No location assigned' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-600">Low Stock Threshold:</span>
                                <span class="text-sm font-medium">{{ number_format($purchase->product->effective_low_stock_threshold) }} units</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-600">Current Stock:</span>
                                <span class="text-sm font-medium {{ $currentWarehouseStock <= $purchase->product->effective_low_stock_threshold ? 'text-red-600' : 'text-green-600' }}">
                                    {{ number_format($currentWarehouseStock, 1) }} units
                                </span>
                            </div>
                            <div class="flex justify-between">
        ...
        <div class="bg-white overflow-hidden shadow-sm rounded-lg">
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    @if($purchaseReal)
                        <a href="{{ route('inventory.show', $purchaseReal) }}" class="bg-green-600 hover:bg-green-700 text-white px-4 py-3 rounded-md text-center font-medium transition-colors duration-200">
                            <svg class="w-5 h-5 inline-block mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if($supplierPurchases->count() > 0)
            <div class="bg-white overflow-hidden shadow-sm rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-medium text-gray-900 mb-4">Recent Goods Receipts from {{ $purchase->supplier_name }}</h3>
